...
* **Ações Implementadas:**
  ...
  4. **Navegação Direta pelos Cards de Resumo:**
     - Tornado os 4 cards do topo (`Total Lançado`, `Aguardando Vínculo`, `Aprovados`, `Reprovados`) inteiramente clicáveis com efeito hover (`cursor: pointer`).
     - Ao clicar em qualquer card, a aba correspondente é ativada dinamicamente e a página realiza uma rolagem suave (`scrollIntoView`) direto até o histórico de lançamentos do filtro selecionado.
  ...
  9. **Módulo de Lançamento Direto "Outros Gastos" no Portal do Colaborador:**
     - Adicionado o ícone/item `📦 Outros` na barra de navegação superior (`layouts/portal.php`) e no grid de ações rápidas (`portal/index.php`).
     - Criada a rota `/portal/outros` (GET e POST) com view dedicada `portal/outros.php` seguindo a mesma dinâmica de Viagens/Combustível.
